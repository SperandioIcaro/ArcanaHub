...
module.exports = {
  ...
  async autocomplete(interaction) {
    ...
    const ativas = await fs
      .readJson(path.join(__dirname, "../fichas/ativas.json"))
      .catch(() => ({}));
    const ativa = ativas[userId];
    if (!ativa) return interaction.respond([]);
    const ficha = await fs
      .readJson(
        path.join(__dirname, "../fichas", ativa.mecanica, userId, ativa.arquivo)
      )
      .catch(() => null);
    ...
    const opcoes = [
      "mão livre",
      equips.arma_principal,
      equips.arma_secundaria,
    ].filter(Boolean);
  ...
  async execute(interaction) {
    ...
    const modoOpt = interaction.options.getString("modo");
    const equips = ficha.equipados || {};
    const todasEquips = [equips.arma_principal, equips.arma_secundaria].filter(
      Boolean
    );
    ...
    if (!isMaoLivre && !todasEquips.some((a) => a.toLowerCase() === escolha)) {
      return interaction.reply({
        content: `❌ Você não está usando **${escolha}**.`,
        ephemeral: true,
      });
    }
    ...
    let armaConfig = null;
    if (!isMaoLivre) {
      const allArmas = require(path.join(
        __dirname,
        "..",
        "mechanics",
        "tormenta20",
      ...
      if (!armaConfig) {
        return interaction.reply({
          content: `❌ Dados da arma **${escolha}** não encontrados.`,
          ephemeral: true,
        });
      }
    }
    ...
    if (!isMaoLivre) {
      if (modoOpt) modo = modoOpt;
        ...
        if (
          /arremesso/.test(props) ||
          /armas de longo alcance|besta|arco|pistola|mosquete/.test(
            armaConfig.categoria
          )
        ) {
          modo = "alcance";
        }
      ...
      if (
        modo === "alcance" &&
        !/arremesso/.test(armaConfig.propriedades.join(" ").toLowerCase())
      ) {
        return interaction.reply({
          content: `❌ **${armaConfig.nome}** não possui ataque de alcance.`,
          ephemeral: true,
        });
      }
    }
    ...
    const isCrit =
      !isMaoLivre && modo === "cc" && nat >= armaConfig.cri_treshold;
    if (nat === 1)
      return interaction.followUp({
        content: "💥 **Erro Crítico!** Você tropeçou na própria espada.",
        ephemeral: true,
      });

    await interaction.followUp({
      content: "🎯 Calculando dano...",
      ephemeral: true,
    });
    await sleep(1000);
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: "Mod Atributo",
          value: `${atributoMod >= 0 ? "+" : ""}${atributoMod}`,
          inline: true,
        },
        {
          name: "Proficiência",
        ...
        {
          name: "Bônus Arma",
          value: `${bonusArma >= 0 ? "+" : ""}${bonusArma}`,
          inline: true,
        },
        { name: "🏆 Total ATA", value: `${totalAtaque}`, inline: true },
        {
          name: "💥 DANO",
          value: `${detalheRolls} = **${danoTotal}**`,
          inline: false,
        }
      )
